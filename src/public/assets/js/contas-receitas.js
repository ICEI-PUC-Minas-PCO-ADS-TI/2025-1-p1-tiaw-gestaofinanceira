    ...
    fetch(apiCategoriasReceitas)
        ...
        .then(contas => {
            ...
            contas.forEach(conta => {
                let addContas = `
                    <input type="radio" 
                        id="conta-receita-${conta.id}" 
                        name="conta" 
                        value="${conta.id}"
                        data-nome="${conta.nome}"
                        data-icon="${conta.icon}">
                    <label for="conta-receita-${conta.id}" class="categoriasReceitas">
                        <div class="cat-icon">
...
btnSelecionarConta.addEventListener('click', () => {
    ...
    if (contaSelecionadaReceita) {
        ...
        const contaId = contaSelecionadaReceita.value; 
        inputConta.value = contaSelecionadaReceita.getAttribute('data-nome');
        inputConta.setAttribute('conta-id', contaId);
