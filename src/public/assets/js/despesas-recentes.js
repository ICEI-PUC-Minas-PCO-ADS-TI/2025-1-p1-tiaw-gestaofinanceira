function exibeDespesas() {
    ...
    .then(([despesasGerais, categoriasDespesa]) => {
        ...
        for (let i = despesas.length - 1; i >= 0 && i >= despesas.length - 4; i--) {
            ...
            <div class="transacao" data-despesa-id="${despesa.id}">
                <div class="transacao-info">
                    <div class="icon-bg bg-recentes-despesa">
                        <i data-lucide="${categoria.icon}" class="icons icon-recentes icon-recentes-despesa"></i>
                    </div>
                    <div class="categoria">
...
const tituloAddDespesa = document.getElementById('tituloAddDespesa');
...
function abrirModalEdicaoDespesa(despesa) {
    ...
    tituloAddDespesa.style.display = 'none';
    editarDespesa.style.display = 'flex';
    ...
    document.getElementById('inputDescricaoDespesa').value = despesa.descricao;
    document.getElementById('inputValorDespesa').value = despesa.valor;
    document.getElementById('dataDespesa').value = despesa.data;
    document.getElementById('inputContaDespesa').value = despesa.conta;
...
function atualizarDespesa(id) {
    const despesaAtualizada = {
        ...
        descricao: document.getElementById('inputDescricaoDespesa').value,
        valor: document.getElementById('inputValorDespesa').value,
        data: document.getElementById('dataDespesa').value,
        categoria: document.getElementById('inputCategoriaDespesa').value,
        conta: document.getElementById('inputContaDespesa').value,
